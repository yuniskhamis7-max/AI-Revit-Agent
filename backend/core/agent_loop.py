...
async def run_agent_loop(
    ...
    Args:
        provider:        Instantiated AIProvider adapter (e.g. Gemini).
        messages:        List of chat history messages in provider-agnostic format:
                         [{"role": "user"|"assistant"|"tool", "content": "..."}]
        tool_schemas:    List of available tool schemas for model function calling.
        execute_tool_fn: Async callback to execute a tool: (tool_name, args) -> result_dict
        max_turns:       Max number of LLM inference turns.
    ...
    while turn < max_turns:
        ...
        for tc in pending_tool_calls:
            ...
            history.append({
                ...
                "content": str(result) if not isinstance(result, dict) else json_dumps(result)
            })
